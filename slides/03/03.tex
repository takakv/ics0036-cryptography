...
\begin{frame}{Block ciphers}
  ...
  \begin{itemize}[<+(1)->]
    \item $E(K, \cdot) : \{0, 1\}^s \times \{0, 1\}^n \to \{0, 1\}^n$
    \item Common notation: $E_K(\cdot) \defeq E(K, \cdot)$
  \end{itemize}
\end{frame}

\begin{frame}{Block cipher versatility}
  A block cipher operation is a keyed shuffle.
  \begin{itemize}[<+(1)->]
    \item Reversible with the key, indistinguishable from random without
    \item Pseudorandom permutation (PRP)
  \end{itemize}
  ...
  \begin{itemize}[<+(1)->]
    \item Stream ciphers
    \item CSPRNGs
    \item MACs \& authenticated encryption (AE)
    \item Key derivation functions (KDF)
  \end{itemize}
\end{frame}
...
\begin{frame}{Initialisation Vector (IV) \& nonce}
  IVs provide randomness and/or uniqueness.
  \begin{itemize}[<+(1)->]
    \item Nonce: number used once (not necessarily random)
    \item Not secret: can be sent with the ciphertext
  \end{itemize}

  \pause
  Attempt at improving ECB:
  \begin{center}
  \begin{tikzpicture}
    \foreach \x in {1, 2, 3} {
      \node (f\x) at ($\x*(2.5cm,0)$) [minimum size=1.25cm,fill=gray!20,draw] {$\ENC$};
...
\begin{frame}{Cipher Block Chaining (CBC) mode}
  ...
  \pause
  CBC requires random and unique IVs!
\end{frame}
...
\begin{frame}{Security level}
  \pause
  The \emph{security level} is a measure of the \enquote{strength} that a primitive achieves.
  ...
  \pause
  For symmetric cryptosystems with $k$-bit keys
  \begin{itemize}[<+(1)->]
    \item classical strength = $k$
    \item quantum resistance is trickier to measure
    \par
    \begin{itemize}
      \item The commonly claimed $k/2$ is overly conservative and not true practice!
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{AES}
  ...
  \pause
  \begin{itemize}
    \item A subset of the Rijndael cipher family
...
\begin{frame}{Attacks on AES}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \item AES-128 is considered \emph{quantum resistant} in practice regardless of key-length
    \par
    \begin{itemize}
      \item See {\footnotesize\url{https://words.filippo.io/128-bits/}} for the rationale
      \item AES-256 may still be recommended or required by standards/guidance
    \end{itemize}
...
\begin{frame}{Data limits}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \item Re-keying needed after $2^{32}$ invocations \href{https://csrc.nist.gov/pubs/sp/800/38/d/final}{[SP 800-38D \S 8.3]}
  \end{itemize}
  ...
  ChaCha20-Poly1305:
  \begin{itemize}[<+(1)->]
    \item Message length limit: $\approx 256$ GiB for 96-bit nonces \href{https://datatracker.ietf.org/doc/html/rfc8439\#section-2.8}{[RFC 8439]}
    \item Re-keying? Some theoretical bounds, but no concrete message count limits
  \end{itemize}
...
\begin{frame}{Disk encryption}
  ...
  \begin{itemize}[<+(1)->]
    \item random access
    \item length-preserving: no room for IVs and tags
  \end{itemize}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \item ESSIV: $IV = E_{\mathrm{hash}(K)}(\mathrm{SN})$ -- unpredictable without the key
  \end{itemize}
\end{frame}

\begin{frame}{Disk encryption (cont.)}
  In practice:
  \begin{itemize}[<+(1)->]
    \item XTS-AES: no chaining, parallel decryption
    \begin{itemize}
      \item BitLocker, FileVault, LUKS, \dots
    \end{itemize}
    \item Google Adiantum: ChaCha-based
    \begin{itemize}
      \item Devices without AES acceleration, e.g. entry-level Android devices
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Keys and passwords}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \item Derive key encryption key from user-provided password (KEK)
