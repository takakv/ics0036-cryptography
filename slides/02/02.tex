...
\begin{frame}{Is it random?}
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item<4-> Source: first 10 fractional digits of $\pi$
    \end{itemize}
    \item<1-> 1918987876
    \begin{itemize}
...
\begin{frame}{Getting randomness on *nix}
  ...
  \begin{itemize}
    ...
    \pause
    \item Both are CSPRNG outputs seeded from the entropy pool
    \pause
...
% Single world
\newcommand{\prngworld}[2]{%
  \draw[rounded corners,thick,fill=red!85]  (0, 0)   rectangle ++(0.8, -3.8);
  \draw[rounded corners,thick,fill=blue!50] (2.6, 0) rectangle ++(3.6, -3.8);

  \node at (0.4, 0.5) {$\AD$};
  \node at (4.4, 0.5) {Challenger};

  \node[worldlabel] at (3.1, 1.3) {#1};

  \draw[->,thick,visible on=<2->] (0.8, -0.9) -- (2.6, -0.9)
    node[arrowlabel] {$\ell$};

  #2

  \draw[<-,thick,visible on=<5->] (0.8, -3.2) -- (2.6, -3.2)
    node[arrowlabel] {$r$};

  \draw[->,thick,visible on=<6->] (0.4, -3.8) decorate[Squiggly]{ -- (0.4, -4.16) };
  \node[visible on=<6->] at (0.4, -4.45) {$\mathit{guess}$};
}

% Two worlds
\newcommand{\prngdiagram}[4]{%
  \begin{tikzpicture}[scale=0.8, transform shape]
    \prngworld{#1}{#3}

    \draw[divider] (6.9, 1.6) -- (6.9, -4.8);
...
\begin{frame}{CSPRNG distinguishing game}
  \begin{figure}
    \centering
    \prngdiagram
      {World $0$: true randomness}
      {World $1$: CSPRNG}
      {\node[visible on=<3->] at (4.4, -2.05) {$r \getsu \{0,1\}^\ell$};}
      {\node[visible on=<4->] at (4.4, -1.8)  {$s \getsu \{0,1\}^k$};
       \node[visible on=<4->] at (4.4, -2.35) {$r \gets \mathsf{CSPRNG}(s, \ell)$};}
  \end{figure}

  \visible<7->{The adversary must output 0 or 1.}
\end{frame}

% \begin{frame}<7>{PRNG distinguishing game}
%   \begin{figure}
%     \centering
%     \prngdiagram{World $?$}{World $?$}{}{}
%   \end{figure}

%   \visible<7->{The adversary does not know which world they are in.}
% \end{frame}

\begin{frame}<7-8>{CSPRNG distinguishing game}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \centering
      \begin{minipage}[c][5.2cm][c]{\linewidth}
        \centering
      ...
      \vspace*{1em}
      The actual world is unknown\dots\\
      {\scriptsize (black box API access only)}
    \end{column}
    \begin{column}{0.4\textwidth}
      ...
      \vspace*{1em}
      \dots{}but the candidates are known.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{IND-CPA game}
  \begin{figure}
    ...
    \begin{tikzpicture}[
      ...
      \draw[rounded corners,thick,fill=blue!50] (5, 0) rectangle ++(3, -3.6);
      \draw[rounded corners,thick,fill=red!85]  (0, 0) rectangle ++(1, -3.6);

      \node at (6.5, 0.5) {Challenger};
      \node at (0.5, 0.5) {$\AD$};

      \node[visible on=<2->] at (6.5, -0.5) {$k \gets \GEN()$};

      \draw[->,thick,visible on=<3->] (1, -1.5) -- (5, -1.5)
        node[arrowlabel] {$m_0, m_1$};

      \node[visible on=<4->] at (6.5, -2.1) {$b \getsu \{0, 1\}$};
      \node[visible on=<4->] at (6.5, -2.6) {$c \gets \ENC_k(m_b)$};

      \draw[<-,thick,visible on=<5->] (1, -3.2) -- (5, -3.2)
        node[arrowlabel] {$c$};

      \draw[->,thick,visible on=<6->] (0.5, -3.6) decorate[Squiggly]{ -- (0.5, -3.96) };
      \node[visible on=<6->] at (0.5, -4.25) {$b'$};

      \def\phasex{8.5}

      \draw[divider,visible on=<3->] (-1, -1) -- (9, -1);
      \node[phaselabel,visible on=<2->] at (\phasex, -0.5) {Setup};
      \node[phaselabel,visible on=<3->] at (\phasex, -2.3) {Challenge};
    \end{tikzpicture}
  \end{figure}

  \visible<7->{$\AD$ succeeds if $b' = b$.}
  
  \visible<8->{If $\lvert{}m_0\rvert{} \neq \lvert{}m_1\rvert{}$ the challenger must abort, i.e. messages must be of equal length.}
\end{frame}
