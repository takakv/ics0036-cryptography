...
\begin{frame}{Secrets and sharing}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \item What if the single party is not trustworthy enough?
  \end{itemize}
...
\begin{frame}{Secret sharing}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \begin{itemize}
      ...
      \item Some schemes do not need the dealer
    \end{itemize}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \begin{figure}
      ...
      \begin{tikzpicture}[scale=1.5,fill opacity=0.4,thick,
        ...
        \fill[fill opacity=1] (pos2) +(0.5, 0.5, -0.5) circle (0.5pt);

        % Draw right cube.
        \path (pos2) + (2,0) coordinate (pos3);
        \foreach \direction in {(0,0,1),(0,1,0),(1,0,0)} {
          \draw[dashed] (pos3) ++ (bul) -- + \direction;
        }

        % Green back line
        % \draw[very thin, dashed, draw=green!40] (pos3) +(0, 0.5, 0) -- +(0, 0.5, -1) -- +(1, 0.5, -1);

        % Back green triangle
        \fill[green!60] (pos3) +(0, 0.5, -1) -- +(0.5, 0.5, -0.5) -- +(1, 0.5, -1) -- +(0, 0.5, -1);
        % Back triangle
        \fill[cyan!60] (pos3) +(1, 1, -1) -- +(0, 1, -1) -- +(0.5, 0.5, -0.5);
        % Right blue pane
        \fill[blue!60] (pos3) +(bmd) -- +(bur) -- +(tur) -- +(tmd);
        % \draw[very thin, draw=blue!40] (pos3) +(1,0,-1) -- +(0.784, 0, -0.784);
        % \draw[very thin, dashed, draw=blue!40] (pos3) +(0,0,0) -- +(0.784, 0, -0.784);

        % Right green triangle back
        \fill[green!60] (pos3) +(1, 0.5, -0.5) -- +(0.5, 0.5, -0.5) -- +(1, 0.5, -1);
        % Left green triangle back
        \fill[green!60] (pos3) +(0, 0.5, -1) -- +(0.5, 0.5, -0.5) -- +(0, 0.5, -0.5);
        % Left triangle
        \fill[cyan!60] (pos3) -- +(0, 1, -1) -- +(0.5, 0.5, -0.5);
        % Right triangle
        \fill[cyan!60] (pos3) -- +(1, 1, -1) -- +(1, 0, 0) -- +(0.5, 0.5, -0.5);
        % Right green triangle front
        \fill[green!60] (pos3) +(1, 0.5, -0.5) -- +(0.5, 0.5, -0.5) -- +(1, 0.5, 0);
        % Left green triangle front
        \fill[green!60] (pos3) +(0, 0.5, 0) -- +(0.5, 0.5, -0.5) -- +(0, 0.5, -0.5);

        % Left cyan dashed
        % \draw[very thin, dashed, draw=cyan!40] (pos3) +(0, 0, 0) -- +(0, 0.784, -0.784);

        % Left blue pane
        \fill[blue!60] (pos3) +(bll) -- +(bmd) -- +(tmd) -- +(tll);
        % Front triangle
        \fill[cyan!60] (pos3) +(blr) -- +(bll) -- +(0.5, 0.5, -0.5);
        % Green triangle front
        \fill[green!60] (pos3) +(0, 0.5, 0) -- +(0.5, 0.5, -0.5) -- +(1, 0.5, 0);

        % Super lines
        % Cyan right line
        % \draw[very thin, draw=cyan!40] (pos3) +(1, 1, -1) -- +(1, 0, 0);
        % Cyan left line
        % \draw[very thin, draw=cyan!40] (pos3) +(0, 1, -1) -- +(0, 0.784, -0.784);
        % Blue top line
        % \draw[very thin, draw=blue!40] (pos3) +(0,1,0) -- +(1, 1, -1);
        % Green front line
        % \draw[very thin, draw=green!40] (pos3) +(0, 0.5, 0) -- +(1, 0.5, 0) -- +(1, 0.5, -1);
        ...
        % Red pane
        \fill[red!60]  (pos3) +(blr) -- +(tlr) -- +(tul) -- +(bul);
        % Left blue pane
        \fill[blue!60] (pos3) +(bll) -- +(bmd) -- +(tmd) -- +(tll);
        ...
        \draw (pos3) +(tll) -- +(tlr) -- +(tur) -- +(tul) -- cycle;
        \draw (pos3) +(tll) -- +(bll) -- +(blr) -- +(bur) -- +(tur) +(blr) -- +(tlr);
        \fill[fill opacity=1] (pos3) +(0.5, 0.5, -0.5) circle (0.5pt);
      \end{tikzpicture}
    \end{figure}
    \item \dots
  \end{itemize}
...
\begin{frame}{Polynomial interpolation}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \item Three distinct points (can) define a quadratic function
    \begin{itemize}
...
\begin{frame}{Issues with Shamir}
  Not verifiable: no pre-emptive detection of fake shares (malicious player or dealer)
  ...
  \begin{columns}
    \begin{column}{.4\textwidth}
      ...
      \begin{itemize}
        \item $t = 4, n = 5$
        \item Manipulated third share
        \item Interpolation yields $f' (\neq f)$
        \item $f(0) = 1 \neq -3 = f'(0)$
      \end{itemize}
    \end{column}
    \begin{column}{.6\textwidth}
      \begin{figure}
        ...
        \addplot[blue!60, thick, domain=-1:6]
        {-(1/3)*x^3 + (5/2)*x^2 - (25/6)*x + 1}
        node[pos=0.77, below, yshift=-4pt] {\scriptsize $f$};

        \addplot[red!60, thick, dashed, domain=-1:6]
        {(1/6)*x^3 - x^2 + (17/6)*x - 3}
        node[pos=0.65, below] {\scriptsize $f'$};

        \addplot[only marks, red!60, mark=square*, mark size=1pt] coordinates {
            (0, -3)
            (1, -1)
            (2, 0)
            (3, 1)
        ...
        \addplot[only marks, blue!60, mark=*, mark size=1pt] coordinates {
            (0, 1)
            (1, -1)
            (2, 0)
            (3, 2)
  ...
  \pause
  \small
  Naive combinatoric detection is not scalable: worst case of $\binom{n}{t}$ comparisons.
  \vspace{-3mm}
  \begin{itemize}
    \footnotesize
    \item Coding-theoretic improvements often applicable
    \vspace{-2mm}
    \item Error correction techniques for Reed-Solomon codes, e.g. Guruswami-Sudan
  \end{itemize}
\end{frame}
...
\begin{frame}{Issues with Shamir}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \end{itemize}
    \item e.g. for ElGamal
    \begin{itemize}
      \item The dealer computes $x \getsu \ZZ_q$ and $\PK \gets g^x$
      \item The dealer shares $x$ between $n$ players and publishes $\PK$
      \item $t$ players collaborate to recover $\SK = x$ needed for decryption: $c_2 \cdot c_1^{-x}$
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Approaches to MPC}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \begin{itemize}
      \item MPC alliance \href{https://web.archive.org/web/20240720125512/https://wiki.mpcalliance.org/protocols.html}{\textit{wiki}}
    \end{itemize}
  \end{itemize}
